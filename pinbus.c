...
module_param(pinbus_enable_clock_gpio,  uint, S_IRUGO | S_IWUSR );
...
static irqreturn_t pinbus_busy_interrupt(int irq, void* data) {
	...
	if ( !pinbus_enable_clock_gpio || gpio_get_value( pinbus_clock_gpio ) ){
		...
		if (pinbus_enable_dbg)
			printk(KERN_ERR "gpio busdata event on irq=%d, data=%c\n", irq, bus_char);

	} else {
		if (pinbus_enable_dbg)
			printk(KERN_ERR "clock is down, ignore irq=%d \n", irq);
	}

	if ( kfifo_len(&pin_dev->fifo) >= pinbus_wake_threshold ){
		wake_up_interruptible(&pin_dev->wait_for_data);
		if (pinbus_enable_dbg)
			printk( KERN_ERR "Wake Reader\n" );
	}
...
static irqreturn_t pinbus_stat_interrupt(int irq, void* data) {
	...
	if (pinbus_enable_dbg)
		printk(KERN_ERR "gpio event stat on irq=%d\n", irq );
	kfifo_in(&pin_dev->fifo,&bus_char, 1);
	if ( kfifo_len(&pin_dev->fifo) >= pinbus_wake_threshold ){
		wake_up_interruptible(&pin_dev->wait_for_data);
		if (pinbus_enable_dbg)
...
static int pinbus_init(void){
	...
	printk(KERN_ERR "Loading Pinbus Module\n");
	...
	printk(KERN_ERR "Loading Pinbus Module successful\n");
	return 0;
...
static void pinbus_exit(void) {
	...
	printk(KERN_ERR "Shutting down Pinbus Module\n");
	free_irq(busy_irq_number, g_pinbus_dev);
	...
	printk(KERN_ERR "Shutting down Pinbus Module successful\n");
